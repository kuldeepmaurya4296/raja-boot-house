  ...
  return (
    <DashboardPage eyebrow="Finance" title="Payouts">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard label="Pending" value={formatINR(currentVendor.payoutsPending)} icon={Wallet} tint="accent" />
        <StatCard label="Paid (YTD)" value={formatINR(76400)} icon={IndianRupee} />
        <StatCard label="Lifetime" value={formatINR(currentVendor.revenue)} icon={IndianRupee} tint="brass" />
      </div>
          ...
          <thead className="bg-muted/60">
            <tr>
              {["Date", "Amount", "Status"].map(h => (
                <th key={h} className="text-left px-5 py-3 text-xs uppercase tracking-wider text-muted-foreground">
                  {h}
                </th>
              ))}
            </tr>
