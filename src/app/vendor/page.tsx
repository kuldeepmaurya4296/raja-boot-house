...
const cols: Column<Order>[] = [
  ...
  { key: "i", header: "Items", render: o => <span className="text-sm">{o.items[0]?.name || "—"}</span> },
  ...
  return (
    <DashboardPage eyebrow={v.name} title="Workshop overview">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Products" value={String(myProducts.length)} delta={2.1} icon={Package} tint="accent" />
        <StatCard label="Orders" value={String(myOrders.length)} delta={11.2} icon={ShoppingCart} tint="brass" />
      ...
      <div className="bg-gradient-to-br from-charcoal to-charcoal/90 text-cream rounded-xl p-6 md:p-8 grid md:grid-cols-3 gap-6 items-center">
        <div className="md:col-span-2">
          <p className="text-[11px] uppercase tracking-widest text-brass font-semibold">Pending payout</p>
