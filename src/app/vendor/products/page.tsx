...
const cols: Column<Product>[] = [
  ...
  { key: "s", header: "Stock", render: p => <span className="text-sm font-semibold">{p.stock}</span> },
  { key: "a", header: "", render: () => <button className="text-xs underline font-semibold cursor-pointer">Edit</button>, className: "text-right" },
];

export default function VendorProductsPage() {
  const mine = products.filter(p => p.vendorId === currentVendor.id);
  return (
    <DashboardPage eyebrow="Inventory" title="My products" action={
