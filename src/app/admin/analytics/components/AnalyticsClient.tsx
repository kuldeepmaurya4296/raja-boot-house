...
export default function AnalyticsClient({
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "financials" | "status_funnel" | "refunds">("overview");
  ...
  const prevOrders = timeframe === "all" ? [] : orders.filter(o => {
  ...
  const calculateDetailedKPIs = (periodOrders: SerializedOrder[]) => {
    ...
    periodOrders.forEach(o => {
      ...
      if (o.status === "CANCELLED") {
        ...
        o.items.forEach(item => {
          ...
          cogs += (price * qty) * 0.60; // 60% base cost of goods
        });
      }
    });
    ...
    periodOrders.forEach(o => {
      if (!["CANCELLED", "RETURNED", "REFUNDED", "RETURN_REQUESTED"].includes(o.status) && o.payment.method !== "COD") {
    ...
    const cancellationRate = periodOrders.length > 0 ? (statusCounts.CANCELLED.count / periodOrders.length) * 100 : 0;
    const returnRate = periodOrders.length > 0 ? ((statusCounts.RETURNED.count + statusCounts.REFUNDED.count + statusCounts.RETURN_REQUESTED.count) / periodOrders.length) * 100 : 0;

    const totalReturnsCount = statusCounts.RETURNED.count + statusCounts.REFUNDED.count + statusCounts.RETURN_REQUESTED.count;
  ...
  const salesDelta = timeframe === "all" ? 0 : getGrowthDelta(currentKPIs.netSales, prevKPIs.netSales);
  const ordersDelta = timeframe === "all" ? 0 : getGrowthDelta(currentKPIs.totalOrders, prevKPIs.totalOrders);
  ...
  const prevCustomersCount = timeframe === "all" ? 0 : customers.filter(c => {
    const d = new Date(c.createdAt);
    return d >= prevStartDate && d < prevEndDate;
  }).length;
  const customersDelta = timeframe === "all" ? 0 : getGrowthDelta(currentCustomersCount, prevCustomersCount);
  ...
  const paymentRevenue: Record<string, number> = { UPI: 0, Card: 0, "Net Banking": 0, Wallet: 0, COD: 0 };
  currentOrders.forEach(o => {
  ...
  const donutSlices = paymentMethodsList.map((method, idx) => {
  ...
  const customerSalesMap: Record<string, { name: string; email: string; orderCount: number; spend: number }> = {};
  currentOrders.forEach(o => {
    if (["CANCELLED", "RETURNED", "REFUNDED"].includes(o.status)) return;
  ...
  const handleExport = () => {
    ...
    if (reportType === "master_ledger") {
      ...
      currentOrders.forEach(o => {
        ...
        if (!dailyMap[day]) {
          dailyMap[day] = { placedCount: 0, grossSales: 0, discounts: 0, cancellations: 0, refunds: 0, netSales: 0, cogs: 0, netProfit: 0 };
        }
        ...
        if (o.status === "CANCELLED") {
          ...
          let orderCogs = 0;
          o.items.forEach(item => {
            orderCogs += ((item.price || 0) * (item.qty || 1)) * 0.60;
          });
      ...
      rows = sortedDays.map(day => {
        ...
        const netSalesVal = stats.grossSales - stats.discounts - stats.cancellations - stats.refunds;
      ...
      rows = currentOrders.map(o => {
        ...
        const orderTotal = o.pricing.total || 0;
        const margin = ["CANCELLED", "RETURNED", "REFUNDED"].includes(o.status) ? 0 : orderTotal - orderCogs;
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard 
          ...
          delta={timeframe === "all" ? 0 : getGrowthDelta(currentKPIs.netProfit, prevKPIs.netProfit)} 
          icon={TrendingUp} 
          tint="brass" 
        />
        <StatCard 
          ...
          delta={timeframe === "all" ? 0 : getGrowthDelta(currentKPIs.totalReturnsValue, prevKPIs.totalReturnsValue)} 
          icon={TrendingDown} 
          tint="muted" 
        />
      </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <span className="text-[10px] uppercase tracking-widest text-muted-foreground">Revenue Over Time</span>
                  <h3 className="font-serif font-bold text-lg text-foreground">Sales Trend</h3>
                </div>
                {salesDelta !== 0 && (
                  <div className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full ${
                    salesDelta >= 0 ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-600"
                  }`}>
              ...
              <div className="relative w-full h-[240px] select-none mt-2" onMouseLeave={() => setHoveredIndex(null)}>
                ...
                {hoveredIndex !== null && chartData[hoveredIndex] && (
                  ...
                  >
                    <div className="font-bold text-[10px] text-brass/75 uppercase tracking-wider">{chartData[hoveredIndex].label}</div>
                    <div className="text-sm font-serif font-bold mt-0.5">{formatINR(chartData[hoveredIndex].revenue)}</div>
                    <div className="text-[10px] text-muted-foreground">{chartData[hoveredIndex].orders} Orders</div>
                  </div>
                )}
            ...
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col justify-between">
              <div>
                <span className="text-[10px] uppercase tracking-widest text-muted-foreground">Sales Distribution</span>
                <h3 className="font-serif font-bold text-lg text-foreground mb-4">Payment Methods</h3>
              </div>

              <div className="flex justify-center items-center py-4">
                <div className="relative w-40 h-40 rounded-full shadow-inner flex items-center justify-center transition-all duration-300" style={{ background: donutGradient }}>
                  <div className="w-28 h-28 bg-card rounded-full flex flex-col items-center justify-center shadow-md">
                    <span className="text-[9px] uppercase tracking-wider text-muted-foreground">Net Sales</span>
                    <span className="text-sm font-serif font-bold mt-0.5">{formatINR(currentKPIs.netSales)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2.5 mt-2">
                {donutSlices.length === 0 ? (
                  <p className="text-xs text-muted-foreground italic text-center">No orders recorded in this period.</p>
                ) : (
                  donutSlices.map(slice => (
                    <div key={slice.name} className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium text-muted-foreground">{formatINR(slice.revenue)} ({slice.percentage}%)</span>
                    </div>
                  ))
                )}
              </div>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-border text-muted-foreground font-semibold">
                      ...
                      <th className="py-2.5 font-bold uppercase tracking-wider text-right">Units Sold</th>
                      <th className="py-2.5 font-bold uppercase tracking-wider text-right">Revenue</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/60">
                    {topProductsList.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="py-4 text-center text-muted-foreground italic">No product sales in this period.</td>
                      </tr>
                    ) : (
                      topProductsList.map(item => (
                        <tr key={item.id} className="hover:bg-muted/30 transition-colors">
                          <td className="py-3 flex items-center gap-3 pr-2">
                            ...
                            <span className="font-semibold text-foreground truncate max-w-[150px] sm:max-w-[200px]">{item.name}</span>
                          </td>
                          <td className="py-3 text-muted-foreground font-medium">{item.brand}</td>
                          <td className="py-3 text-muted-foreground font-medium">{item.category}</td>
                          <td className="py-3 text-right font-semibold text-foreground/80">{item.qtySold}</td>
                          <td className="py-3 text-right font-serif font-bold text-cognac">{formatINR(item.revenue)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {topCustomersList.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic text-center py-6">No orders recorded in this period.</p>
                  ) : (
                    topCustomersList.map((customer, idx) => (
                      <div key={idx} className="flex items-center justify-between border-b border-border/40 pb-3 last:border-0 last:pb-0">
                        <div className="min-w-0 pr-2">
                          <p className="font-semibold text-xs text-foreground truncate">{customer.name}</p>
                          <p className="text-[10px] text-muted-foreground truncate">{customer.email}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="font-serif font-bold text-xs text-cognac">{formatINR(customer.spend)}</p>
                          <p className="text-[10px] text-muted-foreground font-semibold">{customer.orderCount} Orders</p>
                        </div>
                      </div>
                    ))
                  )}
      ...
      {activeTab === "financials" && (
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm max-w-4xl mx-auto">
          <div className="border-b border-border pb-4 mb-6">
            <h3 className="font-serif font-bold text-xl text-foreground">Profit & Loss (P&L) Financial Statement</h3>
            <p className="text-xs text-muted-foreground mt-1">
              Statement of operations for the selected timeframe. All amounts in Indian Rupees (INR).
            </p>
          </div>
          ...
          <div className="space-y-4">
            {/* Revenue section */}
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Operating Revenue</h4>
              <div className="space-y-2">
                <div className="flex justify-between text-sm py-1.5 border-b border-border/40">
                  ...
                  <span className="font-semibold text-foreground">{formatINR(currentKPIs.grossSales)}</span>
                </div>
                <div className="flex justify-between text-sm py-1.5 border-b border-border/40">
                  <span className="text-foreground/80 pl-2">Coupon Discounts Redeemed</span>
                  <span className="font-semibold text-red-500">-{formatINR(currentKPIs.couponDiscounts)}</span>
                </div>
                <div className="flex justify-between text-sm py-1.5 border-b border-border/40">
                  <span className="text-foreground/80 pl-2">Order Cancellations (Value Lost)</span>
                  <span className="font-semibold text-red-500">-{formatINR(currentKPIs.cancellationsValue)}</span>
                </div>
                <div className="flex justify-between text-sm py-1.5 border-b border-border/40">
                  <span className="text-foreground/80 pl-2">Returns & Refunds Deducted</span>
                  <span className="font-semibold text-red-500">-{formatINR(currentKPIs.refundsValue)}</span>
                </div>
            ...
            <div className="pt-4">
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Cost of Goods & Operations</h4>
              <div className="space-y-2">
                <div className="flex justify-between text-sm py-1.5 border-b border-border/40">
                  <span className="text-foreground/80 pl-2">Cost of Goods Sold (COGS) <span className="text-[10px] text-muted-foreground font-normal">(Estimated 60% of item sales price)</span></span>
            ...
            <div className="pt-4">
              <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Expenses & Operational Costs</h4>
              <div className="space-y-2">
                <div className="flex justify-between text-sm py-1.5 border-b border-border/40">
                  <span className="text-foreground/80 pl-2">Shipping Charges Collected</span>
                  <span className="font-semibold text-green-600">+{formatINR(currentKPIs.shippingRevenue)}</span>
                </div>
                <div className="flex justify-between text-sm py-1.5 border-b border-border/40">
                  <span className="text-foreground/80 pl-2">Payment Gateway Fees <span className="text-[10px] text-muted-foreground font-normal">(Estimated 2.0% of online transactions)</span></span>
                  <span className="font-semibold text-red-500">-{formatINR(currentKPIs.gatewayFees)}</span>
                </div>
                <div className="flex justify-between text-sm font-bold bg-primary/10 px-3 py-2.5 rounded-lg border border-primary/20 mt-1.5">
                  ...
                  <span className="text-primary font-serif text-lg font-bold">{formatINR(currentKPIs.netProfit)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-6 pt-4 border-t border-border">
            <div className="bg-muted/40 p-4 rounded-xl border border-border/50 flex flex-col justify-between">
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Gross Profit Margin</span>
                <p className="text-2xl font-bold font-serif text-cognac mt-1">{currentKPIs.grossMargin.toFixed(1)}%</p>
              </div>
              <div className="w-full bg-border h-1.5 rounded-full mt-3 overflow-hidden">
                <div className="bg-cognac h-full rounded-full" style={{ width: `${Math.min(100, Math.max(0, currentKPIs.grossMargin))}%` }} />
              </div>
            </div>
            <div className="bg-primary/5 p-4 rounded-xl border border-primary/10 flex flex-col justify-between">
              <div>
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Net Profit Margin</span>
                <p className="text-2xl font-bold font-serif text-primary mt-1">{currentKPIs.netMargin.toFixed(1)}%</p>
              </div>
              <div className="w-full bg-border h-1.5 rounded-full mt-3 overflow-hidden">
                <div className="bg-primary h-full rounded-full" style={{ width: `${Math.min(100, Math.max(0, currentKPIs.netMargin))}%` }} />
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === "status_funnel" && (
        <div className="space-y-6 max-w-4xl mx-auto">
          <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            <h3 className="font-serif font-bold text-lg text-foreground mb-4">Detailed Order Lifecycle Funnel</h3>
            <p className="text-xs text-muted-foreground mb-6">
              Track conversion and volume flow of orders across all processing stages in the selected timeframe.
            </p>
            
            <div className="space-y-4">
              {Object.entries(currentKPIs.statusCounts).map(([status, item]) => {
                const percentage = currentKPIs.totalOrders > 0 ? (item.count / currentKPIs.totalOrders) * 100 : 0;
                ...
                return (
                  <div key={status} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border/40 pb-3 last:border-0 last:pb-0">
                    ...
                    <div className="flex-1 flex items-center gap-4">
                      ...
                      <div className="w-32 text-right shrink-0">
                        <span className="text-xs font-bold text-foreground">{item.count} orders</span>
                        <span className="text-[10px] text-muted-foreground block font-medium">({percentage.toFixed(1)}% share)</span>
                      </div>
                      <div className="w-24 text-right shrink-0">
                        <span className="text-xs font-bold text-cognac">{formatINR(item.value)}</span>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-card border border-border rounded-xl p-5 shadow-sm">
              <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Order Cancellation Rate</span>
              <div className="flex items-baseline gap-2 mt-2">
                <p className="text-2xl font-bold font-serif text-red-500">{currentKPIs.cancellationRate.toFixed(1)}%</p>
            ...
            <div className="bg-card border border-border rounded-xl p-5 shadow-sm">
              <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Return & Refund Rate</span>
              <div className="flex items-baseline gap-2 mt-2">
                <p className="text-2xl font-bold font-serif text-orange-500">{currentKPIs.returnRate.toFixed(1)}%</p>
                <span className="text-xs text-muted-foreground">of placed orders</span>
              </div>
              <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
      ...
      {activeTab === "refunds" && (
        <div className="space-y-6 max-w-4xl mx-auto">
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col justify-between">
              <div>
                <h3 className="font-serif font-bold text-lg text-foreground mb-4">Refund Channel Preferences</h3>
              ...
              <div className="space-y-4">
                {Object.entries(currentKPIs.refundPrefs).map(([pref, item]) => {
                  const totalRefundValue = Object.values(currentKPIs.refundPrefs).reduce((sum, v) => sum + v.value, 0) || 1;
                  ...
                  return (
                    <div key={pref} className="space-y-1">
                      <div className="flex justify-between text-xs font-semibold">
                        ...
                        <span className="text-muted-foreground">{formatINR(item.value)} ({percentage}%)</span>
                      </div>
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
            ...
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col justify-between">
              <div>
                <h3 className="font-serif font-bold text-lg text-foreground mb-2">Refund Processing Status</h3>
                <p className="text-xs text-muted-foreground mb-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-muted/40 rounded-xl border border-border/50">
                  <div>
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Total Completed Refunds</span>
                    <p className="text-xl font-serif font-bold text-green-600 mt-1">
                      {formatINR(currentOrders.filter(o => o.status === "REFUNDED").reduce((sum, o) => sum + o.pricing.total, 0))}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between p-4 bg-orange-500/5 rounded-xl border border-orange-500/10">
                  <div>
                    <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Total Pending Refunds</span>
                    <p className="text-xl font-serif font-bold text-orange-500 mt-1">
                      {formatINR(currentOrders.filter(o => ["RETURNED", "RETURN_REQUESTED"].includes(o.status)).reduce((sum, o) => sum + o.pricing.total, 0))}
                    </p>
                  </div>
                  <span className="text-xs font-bold bg-orange-500/10 text-orange-500 px-2 py-1 rounded-full">
                    {currentOrders.filter(o => ["RETURNED", "RETURN_REQUESTED"].includes(o.status)).length} Orders
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-semibold">Excel/CSV Exports</span>
        </div>
        <p className="text-xs text-muted-foreground mb-6 max-w-2xl leading-relaxed">
          Generate and export complete structured reports for further analysis. Reports are built on-the-fly based on the currently selected timeframe ({timeframe === "all" ? "All Time" : timeframe === "7d" ? "last 7 days" : timeframe === "30d" ? "last 30 days" : "last 90 days"}).
        </p>
