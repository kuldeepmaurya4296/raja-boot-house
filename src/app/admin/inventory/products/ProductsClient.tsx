...
export function ProductsClient({ products, totalItems = 0 }: { products: ProductData[], totalItems?: number }) {
  ...
  const cols: Column<ProductData>[] = [
    { key: "p", header: "Product", sortKey: "name", render: p => (
    ...
    { key: "pr", header: "Price", sortKey: "price", render: p => <span className="text-sm font-semibold">{formatINR(p.price)}</span> },
    { key: "s", header: "Stock", render: p => <span className={`text-sm font-semibold ${p.stock < 10 ? "text-destructive" : ""}`}>{p.stock}</span> },
    { key: "status", header: "Status", sortKey: "isActive", render: p => (
      <span className={`inline-flex px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider ${p.isActive ? 'bg-emerald-100 text-emerald-800' : 'bg-muted text-muted-foreground'}`}>
